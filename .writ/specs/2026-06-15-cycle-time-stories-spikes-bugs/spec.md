...
> **Status:** Completed ✅ (5/5 stories complete)
> **Created:** 2026-06-15
> **Owner:** @AdamSellke
...
**Deliverable:** Add configurable rolling-window cycle-time metrics for User Stories, Spikes, and Bugs, showing both program-level and per-workstream totals and team averages in business days, with drilldown access to linked work items missing lifecycle data.
...
- **Error experience:** If lifecycle dates are missing, affected items do not distort averages; the UI shows an unavailable-data count for transparency and lets the user open the count to see the linked ADO items behind it.
...
- Unavailable counts must be auditable from the dashboard: clicking an unavailable count opens a list of the affected ADO work items for the selected scope and type.
- Calendar rule for v1 is Monday-Friday business days only, with no holiday calendar.

### Success Criteria

After sync, dashboard users can see rolling-window total cycle time and average cycle time by work item type at both program and workstream levels. Lifecycle-date gaps are visible and can be opened to a linked item list, and tests cover lifecycle date extraction, business-day math, aggregation, API shape, dashboard mapping, configuration behavior, and unavailable-item drilldown.
...
- Lazy-loaded dashboard drilldown for linked unavailable cycle-time work items.
...
- Full unavailable-item audit details beyond linked ADO ID and title.
...
- Use a focused lazy-load API for unavailable item drilldown so the main metrics response does not grow with item-level lists.
...
Unavailable-item details should be loaded from a focused endpoint rather than embedded in the primary metrics payload. The endpoint must reuse the same configured cycle-time window, dashboard/workstream scope, and work item type semantics as the aggregate counts so modal contents match the badge count.
...
Unavailable-data badges should be clickable when their count is greater than zero. Activating a badge opens a modal listing linked ADO work items for the clicked type and scope. The first drilldown version should show minimal detail: linked ADO ID and title.
...
5. `story-5-unavailable-drilldown`: Make unavailable counts clickable and show linked missing-data items. Dependencies: Stories 3 and 4.
