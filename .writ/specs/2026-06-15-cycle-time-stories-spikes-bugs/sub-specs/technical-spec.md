...
7. A focused unavailable-item endpoint lazy-loads linked missing-data items for the clicked type and scope.
...
Story 5 adds a focused unavailable-item drilldown endpoint. It should not add item arrays to the primary metrics payload. The endpoint should:

- Accept a cycle-time type (`UserStory`, `Spike`, `Bug`) and scope context (`program` via dashboard/workstreamIds, or a specific `workstreamId`).
- Resolve the same selected sprint/window anchor and configured `cycleTimeRollingWindow` as `GET /api/metrics`.
- Query unavailable items using the same missing/invalid lifecycle rules that feed `unavailableItemCount`.
- Return minimal display fields: linked ADO ID, title, type, and workstream context.
- Build links without exposing credentials.
...
| Unavailable drilldown query | Badge count and list can drift if filters differ | Reuse metrics API window/scope helpers or centralize query construction | API parity test |
| ADO link construction | Missing org/project config or unsafe credential exposure | Use non-secret configured org/project values and item ID only; return plain item list if URL cannot be built | API/link test |
| Drilldown modal | Endpoint failure or empty result | Existing loading/error/empty dashboard patterns | Component test |
...
| Unavailable drilldown | Badge click opens linked item modal | Missing-date items listed by clicked type/scope | Empty list message if count changed after refresh | Modal error state |
...
| Saved dashboard scope changes between badge render and click | Endpoint uses current request scope; modal count may differ after refresh but should not crash |
| User clicks a badge with zero unavailable items | Badge is not interactive |
...
- API drilldown: unavailable item endpoint for all supported types, program/workstream scope parity, links, empty/error cases.
- UI: adapter formatting, program summary, workstream card breakdown, `N/A` rendering, clickable unavailable modal.
