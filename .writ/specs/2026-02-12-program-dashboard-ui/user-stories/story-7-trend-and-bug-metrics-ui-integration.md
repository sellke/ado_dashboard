...
- [x] Given the bug burndown chart renders, when a user hovers over a sprint bar, then the tooltip shows the count of bugs in `New|Active` states and the count of bugs in `Resolved|Testing|Closed` states. The tooltip labels and chart legend must include the constituent state names (e.g., "Open (New/Active)" and "Closed (Resolved/Testing/Closed)") so the color key and mouseover are aligned. ✅

## Implementation Tasks

- [x] 7.1 Extend dashboard UI types and adapter mappings for trend and bug metric series. ✅ (Still valid)
- [x] 7.1a Update adapter/type documentation to reflect that `activeBugs` represents `New|Active` states and `bugsClosed` represents `Resolved|Testing|Closed` states. ✅
...
- [x] 7.4 Add/refresh component tests for full trend, partial trend, and missing trend states. ✅ (Still valid)
- [x] 7.4a Update component test fixtures to use new bug state groupings (`New|Active` for open, `Resolved|Testing|Closed` for closed). Verify tooltip content reflects these classifications. ✅
...
- Bug burndown chart legend and tooltip labels must include constituent state names to keep them aligned: "Open (New/Active)" and "Closed (Resolved/Testing/Closed)".
- `activeBugs` maps to `New|Active` states; `bugsClosed` maps to `Resolved|Testing|Closed` states.
