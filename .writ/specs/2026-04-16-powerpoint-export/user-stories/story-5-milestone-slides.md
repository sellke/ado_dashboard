...
- [x] Create `lib/export/slides/milestone.ts` — `buildMilestoneSlide(prs, ws: WorkstreamCardViewModel, milestones: ApiMilestoneWithProgress[]): void`
- [x] Filter milestones to this workstream: `const wsMilestones = milestones.filter(m => m.workstreamId === ws.workstreamId)`
- [x] Handle empty `wsMilestones`: render "Milestone data unavailable for this workstream" placeholder text and return
- [x] For each milestone (up to 3): build pptxgenjs `line` chart with 2 series — `Completed SP` (cumulativeCompletedSP per burnupPoint) and `Target SP` (totalSP — flat line at milestone's totalPoints); stacked vertically on slide
- [x] Layout for up to 3 charts stacked: chart heights of ~1.8" each at y positions 0.8, 2.8, 4.8; title per milestone at top of each chart section
- [x] Handle `burnupData.length === 0` for a milestone: render text label only — "{milestone.title}: {percentComplete}% complete"
- [x] For milestones beyond index 2: add text-only entries "· {title}: {percentComplete}% complete" in a list at bottom of slide
- [x] Add slide title: "{WorkstreamName} — Milestones"
- [x] Write unit tests: milestones with data → 2-series chart per milestone; empty milestones → no throw; empty burnupData → text-only label; > 3 milestones → only first 3 get charts
...
- [x] `lib/export/slides/milestone.ts` exists with `buildMilestoneSlide()` exported
- [x] Empty milestones for workstream → "Milestone data unavailable" placeholder, no crash
- [x] Empty burnupData → text-only label with % complete, no crash
- [x] First 3 milestones get line charts; additional milestones are text-only
- [x] Each chart has 2 series: Completed SP (actual burnup) and Target SP (flat line)
