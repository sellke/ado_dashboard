...
- [x] Create `lib/export/slides/velocity.ts` — `buildVelocitySlide(prs, ws: WorkstreamCardViewModel): void`
- [x] Build chart data: three series from `ws.trendSprints` — `Velocity (SP)` (actuals, null for current sprint), `Current Sprint` (null for non-current, value for current), `Rolling Avg` (velocityAvg per sprint)
- [x] Add pptxgenjs `line` chart: x=0.3, y=0.8, w=8.5, h=5.5; x-axis labels = sprint names; `chartColors` for the three series (e.g. `['4c9be8', 'e67700', '868e96']`)
- [x] Add slide title text box: "{WorkstreamName} — Velocity"
- [x] Add metrics text block (right panel): x=9.0, y=0.8, w=4.0 — four lines: "Velocity: {value} SP", "Avg: {rawVelocityRate} SP", "RAG: {rag}", "Predicted: {prediction.velocity} SP"
- [x] Handle null `prediction`: omit predicted line from text block if `ws.prediction === null`
- [x] Handle empty `trendSprints`: skip chart, render "No trend data available" text at center of slide
- [x] Write unit tests: full trendSprints → chart data has 3 series with correct null placement; empty trendSprints → no throw
...
- [x] `lib/export/slides/velocity.ts` exists with `buildVelocitySlide()` exported
- [x] Three chart series built correctly (actuals null for current, current null for actuals, rolling avg for all)
- [x] Empty `trendSprints` produces "No trend data available" text — no crash
- [x] Metrics text block shows velocity value, avg, RAG, and (if available) predicted next sprint
