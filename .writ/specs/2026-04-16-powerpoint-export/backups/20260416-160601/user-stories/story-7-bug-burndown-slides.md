...
- [ ] Create `lib/export/slides/bug-burndown.ts` — export `buildBugBurndownSlide(prs, ws: WorkstreamCardViewModel): void`
- [ ] Build chart data: labels = `ws.trendSprints.map(s => s.sprintName)`; two series:
...
- [ ] Add pptxgenjs `bar` chart with `barGrouping: 'stacked'` at x=0.3, y=0.8, w=8.5, h=5.5; `chartColors: ['c92a2a', '2f9e44']`
- [ ] Add slide title text box: x=0.3, y=0.2, w=12.7, h=0.6 — "{WorkstreamName} — Bug Burndown"
- [ ] Add metrics text block (right panel): x=9.0, y=0.8, w=4.0 — four lines:
...
- [ ] Handle empty `trendSprints`: skip chart and metrics, render "No bug data available" centered text
- [ ] Handle missing current sprint: metrics block shows "–" for current-sprint lines; totals always render
- [ ] Wire `buildBugBurndownSlide` into `lib/export/builder.ts` per-workstream loop, between `buildVelocitySlide` and `buildOverheadSlide`
- [ ] Write unit tests in `__tests__/lib/export/slides.test.ts` (or new `bug-burndown.test.ts`):
...
- [ ] `lib/export/slides/bug-burndown.ts` exists with `buildBugBurndownSlide()` exported
- [ ] Stacked bar chart has exactly 2 series (Open red, Closed green) per sprint
- [ ] Metrics text block shows current-sprint Open/Closed counts and 4-sprint totals
- [ ] Empty `trendSprints` produces "No bug data available" text — no crash
- [ ] All-zero bug counts still render the chart — no crash
- [ ] Missing current sprint in `trendSprints` → metrics block falls back to "–" for current-sprint lines — no crash
- [ ] Unit tests pass for all acceptance criteria cases
- [ ] Wired into `builder.ts` per-workstream loop in the correct ordinal position (between Velocity and Overhead)
