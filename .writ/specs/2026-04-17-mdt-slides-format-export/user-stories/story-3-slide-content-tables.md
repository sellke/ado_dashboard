...
- [x] 3.1 Write unit tests for table row builders (if extracted as pure functions — alternating fill, header row shape).
- [x] 3.2 Sweep `lib/export/slides/*.tsx` for hard-coded colors/fonts; replace with `mdt-theme` / `rag-colors`.
- [x] 3.3 Apply MDT table pattern to all tabular export content; remove vertical borders if present.
- [x] 3.4 Align any KPI tiles or RAG badges that use native shapes with MDT RAG semantic colors.
- [x] 3.5 Verify acceptance criteria; run tests; spot-check deck vs `mockups/`.
...
- [`mockups/mdt-template-slide-reference.png`](../mockups/mdt-template-slide-reference.png) — whitespace / content region proportions (tables/KPIs sit in the white band).  
...
- [x] No stray legacy hex from pre-MDT export in scoped slide files (except chart image pipeline).
- [x] Tables match MDT pattern where tables exist.
- [x] Tests pass; manual spot-check completed.
