...
- [x] 1.1 Write unit tests for `RAG_COLORS` / `ragColor()` — Green, Amber, Red, null, unknown key.
- [x] 1.2 Create `lib/export/mdt-theme.ts` — export MDT palette, typography sizes, content margins, optional wordmark rect; document any intentional deviation from skill coordinates.
- [x] 1.3 Update `lib/export/rag-colors.ts` to use MDT semantic hex from theme or re-export; keep `ragColor` API stable.
- [x] 1.4 Add brief JSDoc on theme exports linking to `.cursor/skills/mdt_slides.md`.
- [x] 1.5 Verify acceptance criteria and run unit tests.
...
- [`mockups/mdt-template-slide-reference.png`](../mockups/mdt-template-slide-reference.png) — full-slide layout context (colors in raster are indicative; **hex values** come from `mdt_slides` / theme).  
...
- [x] Theme module exists and is imported by at least `rag-colors` (or slide stories consume both without duplication).
- [x] All new/changed unit tests pass.
- [x] No regression in export API surface (`buildPresentation` signature unchanged).
