...
- Images: wordmark optional `slide.addImage` from `public/` or imported asset; if missing, omit or use text per spec decision. Prefer **SVG/PDF or high-DPI PNG** for crisp wordmark (see `spec.md` → Visual references).

## Reference screenshot (layout)

- **File:** `../mockups/mdt-template-slide-reference.png` (1280×720).
- **Use:** Structural alignment — left title band, large white content area, slim footer (page # left, metadata string, wordmark bottom-right). Map to inch coordinates under `LAYOUT_WIDE` (13.333" × 7.5") in theme helpers; do not assume pixel parity in PowerPoint.
- **Subtitle:** When empty, do not reserve subtitle text — collapse spacing per `spec.md`.
...
| Empty subtitle | Omit subtitle shape; reduce top margin so content area shifts up (`spec.md` + reference still) |
| Long footer metadata | Truncate/wrap; keep page # visible at left |
