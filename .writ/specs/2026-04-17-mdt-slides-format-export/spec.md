...
### Gold reference screenshot

Stakeholder-provided template still (1280×720 PNG) — **primary layout reference** for title band, blank content area, footer row, and wordmark placement:

![MDT wide template reference — title, subtitle, footer, wordmark](./mockups/mdt-template-slide-reference.png)

| Region / element | Reference content (editable at export time) |
| --- | --- |
| Slide title | `Title with subtitle` (replace with real title; position: top-left band) |
| Subtitle | `Subtitle (optional)` — **omit** the subtitle line when empty; tighten vertical spacing so content area moves up |
| Footer — page # | `{slideNumber}` (left; always visible) |
| Footer — metadata | `{presentationTitle} \| {Month} {DD}, {YYYY} \| Internal use only` (template copy may use the slide-master placeholder string until replaced by export) |
| Wordmark | Medtronic wordmark, bottom-right |

**Observed layout (from reference image, 1280×720):** generous white content canvas; title block ~60–80px from left, ~36px from top; footer ~12–20px above bottom edge; page number at far left; long metadata string left of center; wordmark ~36–48px from right, ~12–20px from bottom. **pptxgenjs** uses `LAYOUT_WIDE` in inches — map these proportions via `mdt-theme` / Story 2 helpers, not ad hoc per slide.

**Typography vs screenshot:** The PNG is a **composition** reference. **Font sizes, weights, and colors** for export remain **normative from `.cursor/skills/mdt_slides.md`** and this spec’s contract (e.g. slide title **32pt regular** navy — not bold; subtitle **14pt bold** black per skill). If the still appears slightly heavier or different in weight, follow the skill, not the raster.

**Wordmark asset:** Prefer a **vector** (SVG/PDF) or high-DPI PNG in `public/` for crisp `addImage`; document format chosen in theme or README.

Additional inventory: `mockups/README.md`, `mockups/component-inventory.md`. Optional: `.writ/docs/design-system.md` where applicable; **mdt_slides** overrides for export-specific chrome.
